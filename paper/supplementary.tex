...
Across all ablation conditions, boundary collapse is the universal
terminal failure mode (Figure~\ref{fig:death_causes}): boundary integrity
($b < 0.1$) consistently reaches the lethal threshold before energy
reaches zero ($e \leq 0.0$), and no age limit is imposed in the default
configuration. This confirms that boundary disintegration is the common
final pathway through which all criterion removals ultimately cause death,
consistent with the convergent failure attractor described in the Discussion.
...
Boundary collapse is the universal terminal cause: all criterion removals
ultimately manifest as boundary disintegration ($b < 0.1$) before energy
depletion or age limits are reached.}
