...
A practical elbow at $k{=}3$ (energy autocorrelation, boundary stability,
homeostasis variance) captures the primary predictive structure
(alive-AUC cross-validated $R^2=0.257$); the best single-target held-out
result is regulation quality ($R^2=0.668$).
\end{abstract}
...
(4)~Phase~1 criterion-reduction analysis identifying a 6-feature minimal
predictor set ($R^2=0.257$ alive-AUC; $R^2=0.668$ regulation quality held-out).
...
topology robustness~(\S\ref{sec:s10robust}), and graded/cyclic experiments~(§S11).}
...
\begin{table}[t]
\caption{Phase~1 Analysis Summary}
\label{tab:phase1}
\small
\begin{tabular}{@{}llr@{}}
\toprule
...
growth$>$evolution as weakest---is directionally consistent with the single-ablation
...
A Phase~2 expansion ($\sim$200 conditions across 5 environment regimes,
with out-of-regime validation) is implemented to provide the statistical
power necessary to confirm or refute these directional findings;
results are reported when available.

\paragraph{Surrogate observables.}
\emph{Regulation quality} is defined as the inverse coefficient of variation
of internal state $s_0$ over the simulation: systems with tight homeostatic
control score high, while those with large fluctuations or drift score low.
...
minimal set (alive-AUC cross-validated $R^2=0.257$; regulation quality
held-out $R^2=0.668$).
