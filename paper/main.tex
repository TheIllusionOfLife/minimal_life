...
Tierra \citep{ray_1991_approach}, Avida \citep{ofria_2004_avida},
Polyworld \citep{yaeger_1994_computational}, Geb \citep{channon_2001_passing},
...
each cover subsets of the seven criteria; no existing system combines
multi-step metabolism with active homeostatic regulation.
Polyworld and Geb each integrate evolution with neural-network controllers
and physical embodiment, but lack explicit metabolic or homeostatic
subsystems amenable to individual-criterion ablation.
...
Full scores for remaining four surveyed systems in the project repository.}
\label{tab:comparison}
\footnotesize
\begin{tabular}{@{}l@{\hskip 4pt}c@{\hskip 4pt}c@{\hskip 4pt}c@{\hskip 4pt}c@{\hskip 4pt}c@{\hskip 4pt}c@{\hskip 4pt}c@{\hskip 4pt}c@{}}
\toprule
System & CO & Me & Ho & Gr & Re & Rs & Ev & $\Sigma$ \\
\midrule
...
\bottomrule
\multicolumn{9}{@{}l@{}}{\scriptsize $^\dagger$Self-assessed. CO=Cell.Org, Me=Metab, Ho=Homeo,}\\
\multicolumn{9}{@{}l@{}}{\scriptsize Gr=Growth, Re=Reprod, Rs=Response, Ev=Evol.}
\end{tabular}
\end{table}

We acknowledge that Table~\ref{tab:comparison} relies on self-assessment;
independent cross-validation by developers of other systems would
strengthen these comparisons.
The five-level rubric used for scoring is defined in~\S S4
...
$d$=1.42 ($\delta$=0.66) at 10,000 steps (§S6), confirming that adaptation
accumulates across generations; cyclic-environment recovery differences
are mixed and treated as supplemental evidence (§S2).
Fitness landscape analysis (\S\ref{sec:s9fitness}) provides additional
evidence: a Jonckheere-Terpstra trend test for directional selection across
generation cohorts and parent-offspring energy regression for heritability.
...
A planned Phase~2 expansion ($\geq$100 conditions including held-out
environment regimes) will provide the statistical power necessary to
confirm or refute these directional findings.
...
(within $\sim$100 steps), followed by boundary decay;
\textbf{Response ablation} shows energy collapse first (organisms cannot
reach resources), then boundary failure;
\textbf{Reproduction ablation} shows a more gradual decline with boundary
integrity breaking first as the un-replaced population ages.
...
metabolism and response ablation; boundary integrity degrades first under
...
(evolution disabled).
Effect size: Cohen's $d$ for evolved vs.\ clonal at the final snapshot.
Success criterion: H1 $p < 0.05$ AND H2 slope 95\% CI excludes zero.
...
seeds each (240 runs total). We report Mann-Whitney $U$ per condition and
Spearman rank correlation between topologies.
