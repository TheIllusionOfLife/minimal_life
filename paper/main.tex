...
proxy control experiments ruling out tautological definitions; and
...
Tierra \citep{ray_1991_approach}, Avida \citep{ofria_2004_avida}, Lenia
\citep{chan_2019_lenia}, Flow-Lenia \citep{plantec_2023_flow}, ALIEN
...
Full scores for remaining four surveyed systems available upon request.}
...
\bottomrule
\multicolumn{9}{l}{\scriptsize $^\dagger$Self-assessment. CO=Cell.Org, Me=Metab, Ho=Homeo,}\\
...
\textit{Full architecture, genome encoding, and parameter specifications appear in~\S S4.}
...
Simulations run for 2,000 steps (sampled every 50) on a 100$\times$100
...
without altering event ordering. Run manifests (\texttt{*\_manifest.json})
...
significant directed predictability for all three hypothesized pathways
...
that the specific implementation shapes system behavior, ruling out
...
pairwise ablation analysis~(§S5), evolution timescale extension~(§S6), and
graded/cyclic experiments~(§S7).}
...
\midrule
PCA & Components ($\geq$90\% var.) & 4 (93\%) \\
Stability (Enet) & Top criterion & Metabolism (0.678) \\
Stability (Enet) & Bottom criterion & Evolution (0.374) \\
Criterion Pareto & $R^2$ direction & All negative (underpow.) \\
Cohen's $d$ & full vs.\ all-off & 10.68 \\
\midrule
Surrogate VIF & Retained / total & 6 / 14 \\
Surrogate Pareto & Elbow at $k$ & $k$=3 \\
...
Right: criterion Pareto curve (all $R^2<0$; underpowered); Enet stability
rankings are primary evidence.}
...
(threshold 10) removes 8 collinear features, retaining 6:
...
Parameter & Value \\
\midrule
World size & $100{\times}100$ toroidal \\
Initial organisms & 30 (25 swarm agents each) \\
Timesteps & 2,000 (sampled every 50) \\
Resource regen. & 0.01/cell/step \\
Point mutation rate & 0.02/gene, scale 0.15 \\
...
disabled still produces measurably lower boundary integrity than the growth-on
...
% ----------------------------------------------------------------------------
\subsection*{S7: Graded and Cyclic Experiments}
